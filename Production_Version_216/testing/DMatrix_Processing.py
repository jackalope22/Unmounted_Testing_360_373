...
"""
NOTE: Lines containing "216" in this file:
Line 188: redis_client.rpush("process_messages_216", "Saving Binary File")
Line 232: redis_client.rpush("process_messages_216", "Collecting Data")
Line 240: redis_client.rpush("process_messages_216", "No Data Collected, Trying Again")
Line 249: redis_client.rpush("process_messages_216", "No Data Collected")
Line 250: redis_client.rpush("process_messages_216", "Turning HV Off")
Line 255: redis_client.rpush("process_messages_216", "Turning HV Off")
Line 261: redis_client.rpush("process_messages_216", "Turning HV Off")
Line 272: redis_client.rpush("process_messages_216", f"Ramping HV up to {hv}")
Line 276: redis_client.rpush("process_messages_216", "Collecting Data")
Line 284: redis_client.rpush("process_messages_216", "No Data Collected, Trying Again")
Line 293: redis_client.rpush("process_messages_216", "No Data Collected")
Line 294: redis_client.rpush("process_messages_216", "Turning HV Off")
Line 299: redis_client.rpush("process_messages_216", "Turning HV Off")
Line 305: redis_client.rpush("process_messages_216", "Turning HV Off")
"""
...
def collect_and_get_counts(collectTime):
    ...
    log.debug(f"binary data collected gm 0: {len(binarydata1)}")
    log.debug(f"binary data collected gm 1: {len(binarydata2)}")
    log.debug(f"binary data collected gm 2: {len(binarydata3)}")
    log.debug(f"binary data collected gm 3: {len(binarydata4)}")
...
def createCSV(datafile, gmid, wonumber, serialnum):
    ...
    if not findpath:
        try: 
            os.makedirs(path)
            with open(f"/media/evfile01/eV common/Production/Test & Measurement Results/keV-360/Unmounted Data Test/{wonumber}/{serialnum}.csv", "w", newline="") as csvfile:
...
def createBinary(datafile, gmid, wonumber, serialnum, testtype):
    redis_client.rpush("process_messages_216", "Saving Binary File")
    ...
    if testtype == 360:
        path = f"/media/evfile01/eV common/Production/Test & Measurement Results/keV-360/Unmounted Data/{wonumber}"
    elif testtype == 373:
        path = f"/media/evfile01/eV common/Production/Test & Measurement Results/keV-373/Unmounted Data/{wonumber}"
...
def pulserScan(wonumber, collectTime, hv, serialnums, testtype):
    ...
    util.turnHVOn(hv)
    redis_client.rpush("process_messages_216", "Collecting Data")
    ...
    if count == 0:
        ...
        redis_client.rpush("process_messages_216", "No Data Collected, Trying Again")
        collect_and_get_counts(collectTime)
        count = 0
        ...
        if count == 0:
            ...
            redis_client.rpush("process_messages_216", "No Data Collected")
            redis_client.rpush("process_messages_216", "Turning HV Off")
            util.turnHVOff()
...
def ThresholdScan(wonumber, collectTime, hv, serialnums, testtype):
    ...
    count = 0
    collections = [binarydata1, binarydata2, binarydata3, binarydata4]
    redis_client.rpush("process_messages_216", f"Ramping HV up to {hv}")
    util.turnHVOn(hv)
