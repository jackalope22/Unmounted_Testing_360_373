...
app = Flask(__name__)
ui = FlaskUI(app=app, server="flask", width=1600, height=1600)
today = datetime.datetime.now().strftime("%Y-%m-%d")
rotatetime = datetime.time(12, 0, 0)
logging.basicConfig(handlers=[TimedRotatingFileHandler(r"Z:\eV common\Production\logging\testing_kev360_373_unmounted_testing_debug_43.log", when="W6", backupCount=5, atTime=rotatetime)],
                    level=logging.DEBUG,
                    datefmt='%Y-%m-%d %H:%M:%S',
